...
class Hungarian:
    ...
    Get result_MPG and total potential after calculation:
    ...
    def get_results(self):
        """Get result_MPG after calculation."""
    ...
    def calculate(self, input_matrix=None, is_profit_matrix=False):
        ...
        while len(self._results) != expected_results:
            ...
            # If number of zeros in the matrix is zero before finding all the result_MPG then an error has occurred.
            if not zero_locations.any():
                raise HungarianError("Unable to find result_MPG. Algorithm has failed.")

            # Find result_MPG and mark rows and columns for deletion
    ...
    def __set_results(self, result_lists):
        """Set result_MPG during calculation."""
        # Check if result_MPG values are out of bound from input matrix (because of matrix being padded).
        # Add result_MPG to result_MPG list.
...
class CoverZeros:
    ...
    def __init__(self, matrix):
        ...
        Run calculation procedure to generate result_MPG.
        """
